...
naspect = len(corpus.aspect_dict)
pad_idx = word2idx['<pad>']
model = PETER(args.peter_mask, src_len, tgt_len, pad_idx, nuser, nitem, ntokens, nsenti, naspect, args.emsize, args.nhead, args.nhid, args.nlayers, args.dropout).to(device)
...
def train(data):
    ...
    total_sample = 0
    while True:
        user, item, rating, seq, aspect, senti = data.next_batch()  # (batch_size, seq_len), data.step += 1
        ...
        optimizer.zero_grad()
        log_word_prob, log_context_dis, _ = model(user, item, senti, aspect, text)  # (tgt_len, batch_size, ntoken) vs. (batch_size, ntoken) vs. (batch_size,)
        ...
        if data.step % args.log_interval == 0 or data.step == data.total_step:
            ...
            print(now_time() + 'context ppl {:4.4f} | text ppl {:4.4f} | {:5d}/{:5d} batches'.format(
                math.exp(cur_c_loss), math.exp(cur_t_loss), data.step, data.total_step))
            context_loss = 0.
            text_loss = 0.
            total_sample = 0
...
def evaluate(data):
    ...
    with torch.no_grad():
        while True:
            ...
            if data.step == data.total_step:
                break
    return context_loss / total_sample, text_loss / total_sample
...
for epoch in range(1, args.epochs + 1):
    ...
    train(train_data)
    val_c_loss, val_t_loss = evaluate(val_data)
    # if args.rating_reg == 0:
    #     val_loss = val_t_loss
    # else:
    val_loss = val_t_loss #+ val_r_loss
    print(now_time() + 'context ppl {:4.4f} | text ppl {:4.4f} | valid loss {:4.4f} on validation'.format(
        math.exp(val_c_loss), math.exp(val_t_loss), val_loss))
...
# Run on test data.
test_c_loss, test_t_loss = evaluate(test_data)
print('=' * 89)
print(now_time() + 'context ppl {:4.4f} | text ppl {:4.4f} | End of training'.format(
    math.exp(test_c_loss), math.exp(test_t_loss)))
